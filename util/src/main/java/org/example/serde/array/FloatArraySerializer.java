...
public class FloatArraySerializer implements Serializer<float[]> {
  ...
  public FloatArraySerializer(CommonSerializer serializer) {
  ...
  public void writeObject(ByteBuf buf, float[] object) {
    SerializerPair pair = serializer.getSerializerPair(Float.TYPE);
    if (pair == null) {
      throw new RuntimeException("类型:" + Float.TYPE + ",未注册");
    }
